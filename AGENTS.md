...
- **Hue Entertainment identity**: preserve both IDs. `entertainment_area_id` is the legacy V1 numeric group used only to activate/deactivate `/groups/<id>/stream`; `entertainment_configuration_id` is the CLIP V2 UUID embedded in every HueStream 2.0 packet. A successful DTLS handshake alone does not prove lights accept packets. Never substitute one ID for the other.
- **Hue V2 certificate boundary**: this Bridge presents a device-specific self-signed HTTPS certificate, so the default Rust TLS client cannot discover V2 configurations by raw IP. Do not disable certificate validation globally. Existing configurations with a persisted V2 UUID stream correctly; retain that UUID on every deploy. Any future automatic re-sync must use bridge-specific pinning or a vendor-supported trust route before it replaces the saved identity.
